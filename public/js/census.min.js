...
s=parseFloat(100*F/currentTotal).toFixed(2),F=B.name.toUpperCase(),H="Ukupno "+H,s="Od ukupnog `"+currentSubSet.split("_").join(" ")+"` skupa : "+(s+"%");c2.fillStyle="rgba(66, 66, 66, 0.8)";c2.beginPath();c2.fillRect(M-1,C-40,8*F.length,14);c2.fillStyle="rgba(66, 66, 66, 0.8)";c2.beginPath();c2.fillRect(M-1,C-30,8*H.length,14);c2.fillStyle="rgba(66, 66, 66, 0.8)";c2.beginPath();c2.fillRect(M-1,C-10,8*s.length,14);c2.font="12px Monospace";c2.fillStyle="#fff";c2.beginPath();c2.fillText(F,M,C-30);c2.fillStyle=
"#fff";c2.beginPath();c2.fillText(H,M,C-20);c2.fillStyle="#fff";c2.beginPath();c2.fillText(s,M,C);lastCountryName=B.name;lastCountryGeometry=B.geometry}},!1);elem.addEventListener("wheel",S);elem.addEventListener("mousedown",function(C){C.target.style.cssText+="cursor: move;";dragging=!0;last_position={x:C.clientX,y:C.clientY}});elem.addEventListener("mouseup",function(C){dragging=!1;var I=C.target.style.cssText,I=I.replace("cursor: move","cursor: default");C.target.style.cssText=I});d3.select(elem).on("mousemove",
V)},I=function(C){if(currentFeatures){var I,B;currentFeatures.forEach(function(F){var H=F.element;F=F.group;H&&("Polygon"==H.geometry.type?gju.pointInPolygon(C,H.geometry)&&!I&&F.pol==currentGender&&(I=H,B=F):"MultiPolygon"==H.geometry.type&&(gju.pointInMultiPolygon(C,H.geometry)&&!I&&F.pol==currentGender)&&(I=H,B=F))});return{name:I?I.properties["NAME_"+currentLevel]:null,geometry:I?I.geometry:null,group:B?B:null}}},C=function(){if(0!=currentLevel)if(wheeling)setTimeout(function(){wheeling&&(wheeling=
!1,C())},200*currentLevel);else if(void 0!=cached[currentLevel])ca(cached[currentLevel]),currentFeatures=cached[currentLevel];else{0==currentLevel&&(currentFeatures=topo.featured.bosnia);1==currentLevel&&(currentFeatures=topo.featured.entities);2==currentLevel&&(currentFeatures=topo.featured.cantons);3==currentLevel&&(currentFeatures=topo.featured.administrative);var I=[];parsed[currentSet].groups.forEach(function(B){var C=Math.random().toString(36).substring(3);currentFeatures.forEach(function(s){var w,
A=B.name,M=!1;if(1==currentLevel){if(-1==["federacija bosne i hercegovine","republika srpska","br\u010dko"].indexOf(A.toLowerCase()))return;w="NAME_"+currentLevel;M=!0}2==currentLevel&&(-1<s.properties.NAME_1.indexOf("Federacija")&&"Canton"==s.properties.ENGTYPE_2&&(w="VARNAME_2",M=!0,"Fo\u010da"==s.properties[w]&&(s.properties[w]="FOCA - FBIH"),-1<s.properties[w].indexOf("ercegbos")&&(s.properties[w]="KANTON 10"),-1<s.properties[w].indexOf("arajev")&&-1<A.indexOf("KANTON SARAJEVO")&&(s.properties[w]=
"KANTON SARAJEVO"),-1<s.properties.NAME_2.indexOf("Podrinje")&&-1<A.indexOf("BOSANSKO")&&(s.properties[w]=A="BOSANSKO PODRINJE"),-1<A.indexOf("NOVO SARAJEVO")&&(M=!1)),-1<s.properties.NAME_1.indexOf("Srpska")&&"Canton"==s.properties.ENGTYPE_2&&(w="NAME_2",M=!1,"Fo\u010da"==s.properties[w]&&(s.properties[w]="FOCA - RS")),-1<s.properties.NAME_1.indexOf("Br\u010dko")&&(w="NAME_2",M=!1));if(3==currentLevel){if(-1<A.indexOf("kanton"))return;"Commune"==s.properties["ENGTYPE_"+currentLevel]&&(w="VARNAME_"+
currentLevel,s.properties[w]||(w="NAME_"+currentLevel),M=!0,-1<s.properties.NAME_1.indexOf("Federacija")&&(-1<s.properties.NAME_3.indexOf("Stari Grad Sarajevo")&&-1<A.indexOf("STARI GRAD SARAJEVO")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Mostar")&&-1<A.indexOf("MOSTAR")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Ustikolina")&&-1<A.indexOf("FO\u010cA - FBiH")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Pale")&&-1<A.indexOf("PALE - FBiH")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Ilid\u017ea")&&
(-1<A.indexOf("ILID")&&-1==A.indexOf("ISTO\u010cNA"))&&(s.properties[w]=A),-1<s.properties[w].indexOf("Trnovo")&&-1<A.indexOf("TRNOVO - FBiH")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Gora\u017ede")&&"GORA\u017dDE"==A&&(s.properties[w]=A),-1<s.properties[w].indexOf("Prozor")&&-1<A.indexOf("PROZOR")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Kupres")&&-1<A.indexOf("KUPRES - FBiH")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Drvar")&&-1<A.indexOf("DRVAR")&&(s.properties[w]=A),-1<
s.properties[w].indexOf("Usora")&&-1<A.indexOf("USORA")&&(s.properties[w]=A="Te\u0161anj / Usora"),-1<s.properties[w].indexOf("Doboj South")&&-1<A.indexOf("DOBOJ-JUG")&&(s.properties[w]=A="DOBOJ-JUG"),-1<s.properties[w].indexOf("Doboj East")&&-1<A.indexOf("DOBOJ-JUG")&&(s.properties[w]=A="DOBOJ-ISTOK"),-1<s.properties[w].indexOf("Sanski Most")&&-1<A.indexOf("SANSKI MOST")&&(s.properties[w]=A="SANSKI MOST")),-1<s.properties.NAME_1.indexOf("Srpska")&&(-1<s.properties[w].indexOf("Mostar")&&-1<A.indexOf("ISTO\u010cNI MOSTAR")&&
(s.properties[w]=A),-1<s.properties[w].indexOf("Srbinje")&&-1<A.indexOf("FO\u010cA - RS")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Pale")&&-1<A.indexOf("PALE - RS")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Ilid\u017ea")&&(-1<A.indexOf("ILID")&&-1<A.indexOf("ISTO\u010cNA"))&&(s.properties[w]=A),-1<s.properties[w].indexOf("Trnovo")&&-1<A.indexOf("TRNOVO - RS")&&(s.properties[w]=A),-1<s.properties[w].indexOf("\u010cajni\u010de")&&-1<A.indexOf("\u010cAJNI\u010cE")&&(s.properties[w]=A=
"\u010cAJNI\u010cE"),-1<s.properties[w].indexOf("Gora\u017ede")&&-1<A.indexOf("NOVO GORA\u017dDE")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Kupres")&&-1<A.indexOf("KUPRES - RS")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Srpski Klju\u010d")&&-1<A.indexOf("RIBNIK")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Srpski Drvar")&&-1<A.indexOf("ISTO\u010cNI DRVAR")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Kne\u017eevo")&&-1<A.indexOf("KNE\u017dEVO")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Srpsko Ora\u0161je")&&
-1<A.indexOf("\u017dABAR")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Brod")&&-1<A.indexOf("BROD")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Gradi\u0161ka")&&-1<A.indexOf("GRADI\u0160KA")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Dubica")&&-1<A.indexOf("DUBICA")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Kostajnica")&&-1<A.indexOf("KOSTAJNICA")&&(s.properties[w]=A),-1<s.properties[w].indexOf("Sanski Most")&&-1<A.indexOf("O\u0160TRA LUKA")&&(s.properties[w]=A="O\u0160TRA LUKA")))}w=
s.properties[w];var K=w.split("|");"[object Array]"===Object.prototype.toString.call(K)&&(w=K[1]?K[1]:K[0]);M&&parser.similarity(A,w,function(w,A,J){simfactor=3===currentLevel?0.98:0.95;w>=simfactor&&I.push({element:s,group:B,id:C})})})});var M=10,B=setInterval(function(){I.length&&(clearInterval(B),ca(I),loadCrap&&loadCrap.remove(),currentFeatures=cached[currentLevel]=I);0==M&&(console.log("[i] gave up, selected dataset is empty or something is wrong."),clearInterval(B));M--},1)}},ca=function(C){var I=
min=currentTotal=0,B="Sve"==currentSubSet?!0:!1;C.forEach(function(C,H){set=C.group;if(set.pol==currentGender){var s=B?parseInt(set.total):parseInt(set.per_set[currentSubSet]);s>I&&(I=s);currentTotal+=s}});selectedLoadCount=Math.round(C.length/3);C.forEach(function(C,H){set=C.group;element=C.element;if(set.pol==currentGender){var s,w;w=B?parseInt(set.total):parseInt(set.per_set[currentSubSet]);domination?(s=Object.keys(set.per_set).sort(function(s,w){return set.per_set[s]-set.per_set[w]}).reverse()[0],
s==currentSubSet?(w=B?parseInt(set.total):parseInt(set.per_set[s]),color=d3.scale.threshold().domain(colorDomain).range(redColors)):color=d3.scale.threshold().domain(colorDomain).range(defaultColors)):color=d3.scale.threshold().domain(colorDomain).range(defaultColors);s=color(100*w/I);c.fillStyle=s;c.beginPath();path(element.geometry);c.fill();loading&&2>=selectedLoadCount&&(loading=!1,c2.clearRect(0,0,width,height),c2.clearRect(5,5,133,14));selectedLoadCount--}})},S=function(C){loading=wheeling=
!0;var I=1250;scale=projection.scale();3E4<scale?projection.scale(scale+-I):8E3>scale?(I=+I,projection.scale(scale+I)):(I=0>C.deltaY?+I:-I,projection.scale(scale+I),ha())},V=function(C){if(dragging){if("undefined"!=typeof last_position.x){loading=wheeling=!0;var I=last_position.x-d3.event.clientX,B=last_position.y-d3.event.clientY;C=projection.rotate();var F=projection.scale();scale=d3.scale.linear().domain([-1*F,F]).range([-90,90]);I=scale(I);B=scale(B);C[0]-=I;C[1]+=B;-18.5>C[0]&&(C[0]=-18.5);-13.5<
C[0]&&(C[0]=-13.5);-42.5<C[1]&&(C[1]=-42.5);-45.5>C[1]&&(C[1]=-45.5);projection.rotate(C);0==currentLevel&&(currentFeatures=topo.featured.bosnia);1==currentLevel&&(currentFeatures=topo.featured.entities);2==currentLevel&&(currentFeatures=topo.featured.cantons);3==currentLevel&&(currentFeatures=topo.featured.administrative);ha()}last_position={x:d3.event.clientX,y:d3.event.clientY}}},da=function(){now=Date.now();elapsed=now-then;elapsed>fpsInterval&&(then=now-elapsed%fpsInterval,moved&&requestAnimationFrame(da))},
ha=function(){c.save();c.setTransform(1,0,0,1,0,0);c.clearRect(0,0,width,height);c.restore();loading&&(c2.fillStyle="rgba(66, 66, 66, 0.8)",c2.beginPath(),c2.fillRect(5,5,133,14),c2.font="12px Monospace",c2.fillStyle="#fff",c2.beginPath(),c2.fillText("U\u010cITAVANJE ...",10,15));c.strokeStyle="#333";c.lineWidth=0.5;c.beginPath();path.context(c)(graticule());c.stroke();c.strokeStyle="#000";c.lineWidth=2;c.beginPath();path(topo.meshed.bosnia);c.stroke();c.strokeStyle="#000";c.lineWidth=1.5;c.beginPath();
path(topo.meshed.entities);c.stroke();c.strokeStyle="#000";c.lineWidth=1.2;c.beginPath();path(topo.meshed.cantons);c.stroke();c.strokeStyle="#000";c.lineWidth=1;c.beginPath();path(topo.meshed.administrative);c.stroke();defaultColors.forEach(function(C,I){c.strokeStyle="#dfdfdf";c.lineWidth=1;c.beginPath();c.fillStyle=C;c.rect(width-defaultColors.length*(I/1.5)-15,height-12,10,10);c.stroke();c.fill()});domination&&redColors.forEach(function(C,I){c.strokeStyle="#dfdfdf";c.lineWidth=1;c.beginPath();
c.fillStyle=C;c.rect(width-redColors.length*(I/1.5)-15,height-25,10,10);c.stroke();c.fill()});C()};return{load:function(){loadCrap=document.createElement("div");loadCrap.id="loading";loadCrap.innerHTML="U\u010ditavanje ...";document.body.appendChild(loadCrap);projection=d3.geo.satellite().distance(1.5).scale(9797).rotate([-16.560000000000002,-43.56,4.999999999999998]).center([1,1]).tilt(7).clipAngle(180*Math.acos(1/1.1)/Math.PI-1E-6).precision(0.1);graticule=d3.geo.graticule().extent([[-90,-80],[90,
80]]).step([2,2]);canvas=d3.select(".holder").append("canvas").attr("width",width).attr("height",height).attr("id","renderLayer");c=canvas.node().getContext("2d");path=d3.geo.path().projection(projection).pointRadius(1.5).context(c);elem=document.querySelector("canvas");elemLeft=elem.offsetLeft;elemTop=elem.offsetTop;context=elem.getContext("2d");color=d3.scale.threshold().domain(colorDomain).range(defaultColors);topo={bosnia:null,entities:null,cantons:null,administrative:null};queue().defer(d3.json,
"/bosnia-and-herzegovina/BIH_adm0.topojson").defer(d3.json,"/bosnia-and-herzegovina/BIH_adm1.topojson").defer(d3.json,"/bosnia-and-herzegovina/BIH_adm2.topojson").defer(d3.json,"/bosnia-and-herzegovina/BIH_adm3.topojson").defer(d3.json,"/sve").await(na)}}}();var width=750,height=650,c,c2,projection,canvas,path,graticule,elem,elemLeft,elemTop,context,lastCountryName="",lastCountryGeometry=null,frameCount=0,fps,fpsInterval=1E3/fps,then=Date.now(),startTime=then,moved=!0,sets=[],shadedRegions=[],colorDomain=[0.01,1,2,3,4,5,6,7,10,15,20,25,35,45,51,70,80,90,95,100],defaultColors="#ffffe0 #f7fc94 #e7f87e #d7f273 #c9ec6b #bbe666 #ace061 #a0d95d #92d258 #86cc54 #7bc650 #70be4b #65b745 #5ab13f #4faa39 #44a333 #3a9c2b #2f9523 #238d1a #14870e #008000".split(" "),
